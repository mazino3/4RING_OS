...
SYS_OBJS = init/init.o init/main.o core/core.o devs/devs.o libs/libs.o users/users.o \
	core/gdt_mem.o

all: apps/apps.o Image
...
sys_out/system: $(SYS_OBJS)
	$(LD) -M -T ld_core.scr  $(SYS_OBJS) -z muldefs -o sys_out/core.o > core.lnk
...
make_iso: sys_out/sys_load.o
	...
	$(CP) $(LIBS) $(BOOT_PATH)/$(LIBS_RAM)
	$(CP) $(USRS) $(BOOT_PATH)/$(USRS_RAM)
	$(CP) $(APPS) $(BOOT_PATH)/$(APPS_RAM)
...
make_img:
	...
	$(CP) $(LIBS) $(IMG_PATH)/$(LIBS_RAM)
	$(CP) $(USRS) $(IMG_PATH)/$(USRS_RAM)
	$(CP) $(APPS) $(IMG_PATH)/$(APPS_RAM)
